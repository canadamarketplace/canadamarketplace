...
export default function HomePage({ scrollTo }: { scrollTo?: string }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-24 lg:pt-28 lg:pb-32">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-base sm:text-lg lg:text-xl text-cm-muted max-w-2xl mx-auto mb-10 leading-relaxed">
              {t('home.heroSubtitle')}
            </p>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-3xl mx-auto">
              {stats.map((stat) => (
                <div key={stat.label} className="text-center">
                  <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-cm-primary">
                  ...
                  <p className="text-xs sm:text-sm text-cm-dim mt-1">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-20 lg:py-24 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-cm-muted max-w-xl mx-auto">
              {t('home.safetyDesc')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {safetyFeatures.map((feature) => (
              <Card key={feature.title} className="bg-cm-elevated backdrop-blur-xl border-cm-border-subtle rounded-2xl hover:border-cm-border-hover transition-all group">
                <CardContent className="p-6">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500/10 to-red-500/10 border border-cm-border-subtle flex items-center justify-center mb-4 group-hover:from-red-500/20 group-hover:to-red-500/20 transition-all">
                    <feature.icon className="w-6 h-6 text-red-400" />
                  </div>
                  <h3 className="text-base font-semibold text-cm-primary mb-2">{feature.title}</h3>
                  <p className="text-sm text-cm-dim leading-relaxed">{feature.desc}</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      {/* Categories */}
      <section ref={categoriesRef} id="categories" className="py-20 lg:py-24 bg-cm-deep">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-cm-muted">{t('home.browseByCategoryDesc')}</p>
          </div>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className="group p-5 rounded-2xl bg-cm-elevated backdrop-blur-xl border border-cm-border-subtle hover:border-red-500/20 transition-all text-left hover:-translate-y-1"
                >
                  <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-red-500/10 to-red-600/5 border border-cm-border-subtle flex items-center justify-center mb-3 group-hover:from-red-500/20 group-hover:to-red-600/10 transition-all">
                    <Icon className="w-5 h-5 text-red-400" />
                  </div>
                  <h3 className="text-sm font-semibold text-cm-secondary mb-1">{cat.name}</h3>
                  <p className="text-xs text-cm-faint">{cat.count.toLocaleString()} {t('home.items')}</p>
                </button>
              )
            })}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-cm-muted mt-1">{t('home.handpicked')}</p>
            </div>
            <Button
              variant="outline"
              onClick={() => navigate('browse')}
          ...
          {loading ? (
            ...
            <div className="text-center py-16 rounded-2xl bg-cm-elevated border border-cm-border-subtle">
              <Package className="w-12 h-12 text-cm-faint mx-auto mb-4" />
              <p className="text-cm-dim">{t('home.noFeaturedProducts')}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {featured.map((product) => {
                ...
                return (
                  <button
                    ...
                    className="group rounded-2xl bg-cm-elevated backdrop-blur-xl border border-cm-border-subtle hover:border-cm-border-hover overflow-hidden transition-all text-left hover:-translate-y-1"
                  >
                    ...
                    <div className="p-4">
                      <h3 className="text-sm font-medium text-cm-secondary truncate group-hover:text-cm-primary transition-colors">{product.title}</h3>
                      <p className="text-xs text-cm-faint mt-0.5 truncate">{product.store.name}</p>
      ...
      <section ref={regionsRef} id="regions" className="py-20 lg:py-24 bg-cm-deep">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-cm-muted">{t('home.shopAcrossCanadaDesc')}</p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {PROVINCES.map((province) => (
              ...
              >
                ...
                {province.name}
                <span className="text-cm-faint ml-1">{province.code}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl bg-gradient-to-br from-red-900/15 via-neutral-900/60 to-red-900/10 border border-cm-border-subtle p-8 lg:p-12 relative overflow-hidden">
            ...
            <div className="relative z-10 grid lg:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-cm-muted leading-relaxed mb-6">
              ...
              <div className="hidden lg:grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={loc.province} className="p-3 rounded-xl bg-cm-hover border border-cm-border-subtle hover:border-cm-border-hover transition-all">
                    <span className="text-lg mb-1 block">{loc.icon}</span>
                    <p className="text-xs font-medium text-cm-secondary truncate">{loc.city}</p>
                    <p className="text-[10px] text-cm-faint">{loc.province}</p>
      ...
      <section ref={howItWorksRef} id="how-it-works" className="py-20 lg:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-cm-muted">{t('home.simpleSafeCanadian')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((step, i) => (
              <div key={step.title} className="relative text-center group">
                {i < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-10 left-[60%] w-[80%] border-t border-dashed border-cm-border-hover" />
                )}
                <div className="relative inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-red-500/10 to-red-500/10 border border-cm-border-subtle mb-4 group-hover:from-red-500/20 group-hover:to-red-500/20 transition-all">
                ...
                <h3 className="text-base font-semibold text-cm-primary mb-2">{step.title}</h3>
                <p className="text-sm text-cm-dim max-w-[200px] mx-auto leading-relaxed">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={sellerCtaRef} id="seller-cta" className="py-20 lg:py-24 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto">
            <div className="rounded-3xl bg-gradient-to-br from-neutral-900/80 to-neutral-900/40 backdrop-blur-xl border border-cm-border-subtle p-8 lg:p-12">
              <div className="grid lg:grid-cols-2 gap-8 items-center">
                <div>
                  ...
                  <p className="text-cm-muted mb-6 leading-relaxed">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    <div key={item.label} className="text-center p-4 rounded-2xl bg-cm-hover border border-cm-border-subtle">
                      <item.icon className="w-5 h-5 text-red-300 mx-auto mb-2" />
                      <p className="text-xl font-bold text-cm-primary">{item.value}</p>
                      <p className="text-[10px] text-cm-faint mt-1">{item.label}</p>
                    </div>
                  ))}
                </div>
              </div>
