...
export default function Footer() {
  ...
  return (
    <footer className="bg-cm-deep border-t border-cm-border-subtle">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 lg:gap-8">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-xs text-cm-dim leading-relaxed mb-4">
              {t('footer.tagline')}
            </p>
          ...
          {sections.map((section) => (
            <div key={section.title}>
              <h4 className="text-sm font-semibold text-cm-secondary mb-4">{section.title}</h4>
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <button
                      onClick={() => navigate(link.page)}
                      className="text-xs text-cm-dim hover:text-cm-secondary transition-colors"
                    >
                      {link.label}
                    </button>
                  </li>
        ...
        <div className="mt-12 pt-8 border-t border-cm-border-subtle flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-cm-faint">
            &copy; {new Date().getFullYear()} Canada Marketplace. {t('footer.allRightsReserved')}
          </p>
          <div className="flex items-center gap-1 text-xs text-cm-faint">
            <span>🍁</span>
